 ...
 * Lindgren et al.
 */
public class ProphetRouterEnergyJeje extends EnergyAwareRouter {
     ...
     */
    public static final String PROPHET_NS = "ProphetRouterEnergyJeje";
    /**
     ...
     */
    public ProphetRouterEnergyJeje(Settings s) {
        super(s);
     ...
     */
    protected ProphetRouterEnergyJeje(ProphetRouterEnergyJeje r) {
        super(r);
    ...
    private void updateTransitivePreds(DTNHost host) {
        ...
        assert otherRouter instanceof ProphetRouterEnergyJeje : "PRoPHET only works "
                + " with other routers of same type";

        double pForHost = getPredFor(host); // P(a,b)
        Map<DTNHost, Double> othersPreds
                = ((ProphetRouterEnergyJeje) otherRouter).getDeliveryPreds();
    ...
    private Tuple<Message, Connection> tryOtherMessages() {
        ...
        for (Connection con : getConnections()) {
            ...
            ProphetRouterEnergyJeje othRouter = (ProphetRouterEnergyJeje) other.getRouter();

            if (othRouter.isTransferring()) {
                continue; // skip hosts that are transferring
            }

            if (this.getCurrentEnergy() < othRouter.getCurrentEnergy()) {
                for (Message m : msgCollection) {
                    if (othRouter.hasMessage(m.getId())) {
                        continue; // skip messages that the other one has
                    }
                    if (!othRouter.hasMessage(m.getId()) && othRouter.getPredFor(m.getTo()) > getPredFor(m.getTo())) {
    ...
    private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
        ...
        public int compare(Tuple<Message, Connection> tuple1,
            ...
            double p1 = ((ProphetRouterEnergyJeje) tuple1.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple1.getKey().getTo());
            // -"- tuple2...
            double p2 = ((ProphetRouterEnergyJeje) tuple2.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple2.getKey().getTo());
...
//        ProphetRouterEnergyJeje r = new ProphetRouterEnergyJeje(this);
//        return r;
//    }
