 ...
 * Lindgren et al.
 */
public class ProphetRouterBufferJeje extends ActiveRouter {
     ...
     */
    public ProphetRouterBufferJeje(Settings s) {
        super(s);
     ...
     */
    protected ProphetRouterBufferJeje(ProphetRouterBufferJeje r) {
        super(r);
    ...
    private void updateTransitivePreds(DTNHost host) {
        ...
        assert otherRouter instanceof ProphetRouterBufferJeje : "PRoPHET only works "
                + " with other routers of same type";

        double pForHost = getPredFor(host); // P(a,b)
        Map<DTNHost, Double> othersPreds
                = ((ProphetRouterBufferJeje) otherRouter).getDeliveryPreds();
    ...
    private double getCurrentEnergy(DTNHost h) {
    ...
    private Tuple<Message, Connection> tryOtherMessages() {
        ...
        for (Connection con : getConnections()) {
            ...
            ProphetRouterBufferJeje othRouter = (ProphetRouterBufferJeje) other.getRouter();

            if (othRouter.isTransferring()) {
                continue; // skip hosts that are transferring

            }
            int freeBufferSize = othRouter.getBufferSize();

            if (getCurrentEnergy(this.getHost()) < getCurrentEnergy(other)) {
                for (Message m : msgCollection) {
                    if (othRouter.hasMessage(m.getId())) {
                        continue; // skip messages that the other one has
                    }
                    tryAllMessagesToAllConnections();
                    if (othRouter.getPredFor(m.getTo()) > getPredFor(m.getTo()) && othRouter.getFreeBufferSize() < messages.size()) {//coding disini
                        // the other node has higher probability of delivery
                        messages.add(new Tuple<Message, Connection>(m, con));//add msglist
                    }
                    freeBufferSize--;
                }
...
//        }
//
//        int freeBufferSize = othRouter.getFreeBufferSize();
//
//        if (getCurrentEnergy(this.getHost()) < getCurrentEnergy(other)) { // Check if current node has less energy
//            for (Message m : msgCollection) {
//                if (othRouter.hasMessage(m.getId())) {
//                    continue; // skip messages that the other one has
//                }
//
//                // Check delivery probability and buffer space
//                if (othRouter.getPredFor(m.getTo()) > getPredFor(m.getTo()) && freeBufferSize > m.getSize()) {
//                    // The other node has higher probability of delivery and enough buffer space
//                    messages.add(new Tuple<Message, Connection>(m, con)); // Add message to send list
//                    freeBufferSize -= m.getSize(); // Update buffer space
//                }
//            }
//        }
//    }
...
//}
    ...
    private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
        ...
        public int compare(Tuple<Message, Connection> tuple1,
            ...
            double p1 = ((ProphetRouterBufferJeje) tuple1.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple1.getKey().getTo());
            // -"- tuple2...
            double p2 = ((ProphetRouterBufferJeje) tuple2.getValue().
                    getOtherNode(getHost()).getRouter()).getPredFor(
                    tuple2.getKey().getTo());
    ...
    public MessageRouter replicate() {
        ProphetRouterBufferJeje r = new ProphetRouterBufferJeje(this);
        return r;
    }
